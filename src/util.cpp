...
const char * const BITCOIN_CONF_FILENAME = "tecax.conf";
const char * const BITCOIN_PID_FILENAME = "tecaxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tecax" is a composite category enabling all Tecax-related debug output
            if(ptrCategory->count(string("tecax"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TecaxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TecaxCore
    // Mac: ~/Library/Application Support/TecaxCore
    // Unix: ~/.tecaxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TecaxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TecaxCore";
#else
    // Unix
    return pathRet / ".tecaxcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty tecax.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override tecax.conf
